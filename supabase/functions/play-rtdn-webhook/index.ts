...
Deno.serve(async (req: Request) => {
  ...
  if (!sub) {
    ...
    console.warn(`play-rtdn-webhook: unknown purchase_token (first seen via RTDN)`);
    return json({ ok: true }, 200);
  }
  ...
  // ── 5. Optionally re-fetch expiry from Google Play for renewals ───────────
  let newPeriodEnd: string | null = null;
  const saJson = Deno.env.get('GOOGLE_PLAY_SERVICE_ACCOUNT_JSON');
  if (saJson && subscriptionId &&
      (notificationType === NT.SUBSCRIPTION_RENEWED || notificationType === NT.SUBSCRIPTION_RECOVERED)) {
    try {
      const accessToken = await getGoogleAccessToken(saJson);
      const packageName = Deno.env.get('PLAY_PACKAGE_NAME') ?? 'com.budgetplanner.app';
      const info = await fetchPlaySubscription(packageName, subscriptionId, purchaseToken, accessToken);
      if (info?.expiryTimeMillis) {
        newPeriodEnd = new Date(parseInt(info.expiryTimeMillis, 10)).toISOString();
      }
    } catch (err) {
  ...
  if (update.subscriptionStatus !== null) {
    const subUpdate: Record<string, unknown> = { status: update.subscriptionStatus };
    if (newPeriodEnd) subUpdate.current_period_end = newPeriodEnd;
  ...
  if (update.profileSubscriptionStatus !== null) profileUpdate.subscription_status = update.profileSubscriptionStatus;
