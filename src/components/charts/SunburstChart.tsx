...
interface SunburstChartProps {
  catBreakdown: BreakdownItem[];
  catTrendData: Record<string, number | string>[] | null | undefined;
  dark: boolean;
}

export default function SunburstChart({ catBreakdown, catTrendData, dark }: SunburstChartProps) {
  const W = 210, H = 210, cx = 105, cy = 105;
  if (!catBreakdown.length) return null;
  const total = catBreakdown.reduce((s, d) => s + d.value, 0);
  if (!total) return null;
  const arc = (sa: number, ea: number, r1: number, r2: number) => {
  ...
  let a0 = -Math.PI / 2;
  const inner: { sa: number; ea: number; color: string }[] = [];
  const outer: { sa: number; ea: number; color: string; op: number }[] = [];
  catBreakdown.forEach(cat => {
    const span = (cat.value / total) * 2 * Math.PI, a1 = a0 + span;
    inner.push({ sa: a0, ea: a1, color: cat.color });
    const mVals = catTrendData ? catTrendData.map(r => (r[cat.name] as number) || 0) : [];
