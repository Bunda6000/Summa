...
interface ExpenseModalProps {
  ...
  monthKey: string;
  monthLabel: string;
  entry: ExpenseEntry | null | undefined;
  catMaxYear: number;
  suggestedDay: number;
  onSave: (entry: ExpenseEntry, applyMonths: number) => void;
  onDelete: () => void;
  onReorderSubs?: (newSubs: Subcategory[]) => void;
  onClose: () => void;
}

export default function ExpenseModal({ catObj, monthKey, monthLabel, entry, catMaxYear, suggestedDay, onSave, onDelete, onReorderSubs, onClose }: ExpenseModalProps) {
  const [subOrder, setSubOrder] = useState<Subcategory[]>(catObj.subcategories || []);
  const hasSubs = subOrder.length > 0;
  const [amount, setAmount] = useState<string | number>(entry?.amount ?? "");
  const [subAmounts, setSubAmounts] = useState<Record<string, string | number>>(entry?.subAmounts || {});
  const [subPaid, setSubPaid] = useState<Record<string, { paid: boolean; paidDate: string }>>(entry?.subPaid || {});
  const [fields, setFields] = useState<Record<string, string>>(entry?.fields || {});
  const [extras, setExtras] = useState<{ name: string; value: string }[]>(entry?.extras || []);
  ...
  const setField = (fid: string, val: string) => setFields(prev => ({ ...prev, [fid]: val }));
  const setSubAmt = (sid: string, val: string | number) => setSubAmounts(prev => ({ ...prev, [sid]: val }));
  const toggleSubPaid = (sid: string, checked: boolean) => {
    setSubPaid(prev => ({ ...prev, [sid]: { paid: checked, paidDate: checked ? (prev[sid]?.paidDate || today()) : "" } }));
  };
  const setSubPaidDate = (sid: string, date: string) => {
    setSubPaid(prev => ({ ...prev, [sid]: { ...prev[sid], paid: true, paidDate: date } }));
  };

  const handleSubReorder = (from: number, to: number) => {
  ...
  const updateExtra = (i: number, key: 'name' | 'value', val: string) => setExtras(prev => prev.map((e, ei) => ei === i ? { ...e, [key]: val } : e));
  const removeExtra = (i: number) => setExtras(prev => prev.filter((_, ei) => ei !== i));

  const computedTotal = hasSubs
    ? subOrder.reduce((s, sc) => s + (evalExpr(String(subAmounts[sc.id] ?? "")) || 0), 0)
    : evalExpr(String(amount ?? "")) || 0;

  /* Derive overall paid status from subcategories */
  const allSubsPaid = hasSubs && subOrder.every(sc => subPaid[sc.id]?.paid && (evalExpr(String(subAmounts[sc.id] ?? "")) || 0) > 0);
  const paidSubTotal = hasSubs ? subOrder.reduce((s, sc) => s + (subPaid[sc.id]?.paid ? (evalExpr(String(subAmounts[sc.id] ?? "")) || 0) : 0), 0) : 0;

  const handleSave = () => {
    if (hasSubs) {
      if (computedTotal <= 0) return;
      const cleanSub: Record<string, number> = {};
      subOrder.forEach(sc => { const v = evalExpr(String(subAmounts[sc.id] ?? "")); if (v > 0) cleanSub[sc.id] = v; });
      const cleanSubPaid: Record<string, { paid: boolean; paidDate: string }> = {};
      ...
      const a = evalExpr(String(amount ?? ""));
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ background: "var(--chip2)", borderRadius: 10, padding: 12 }}>
            {subOrder.map((sc, si) => {
              ...
              const hasAmt = (evalExpr(String(subAmounts[sc.id] ?? "")) || 0) > 0;
              return (
                <div key={sc.id} draggable
                  ...
                  <input type="text" inputMode="decimal" value={subAmounts[sc.id] ?? ""} onChange={e => setSubAmt(sc.id, e.target.value)}
                    ...
                    onKeyDown={e => { if (e.key === "Enter") { const v = evalExpr((e.target as HTMLInputElement).value); if (!isNaN(v) && v >= 0) setSubAmt(sc.id, v); } }}
