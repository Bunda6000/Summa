...
export default function DatePicker({ value, onChange, onBlur, autoFocus, style, monthKey, suggestedDay, compact }: DatePickerProps) {
  ...
  const findModalEl = (el: HTMLElement | null): HTMLElement | null => {
  ...
  const commit = (iso: string) => { if (iso) { onChange(iso); setText(fmtDate(iso)); setOpen(false); } };
  const commitEu = (v: string) => { const iso = parseEuDate(v); if (iso) commit(iso); };
  ...
  const pickCalDay = (y: number, m: number, d: number) => {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      const insideWrap = wrapRef.current && wrapRef.current.contains(e.target as Node);
      const insidePanel = panelRef.current && panelRef.current.contains(e.target as Node);
  ...
  let week: (number | null)[] = new Array(calStart).fill(null);
  ...
  return (
    <div ref={el => { wrapRef.current = el; }} style={{ position: "relative", display: "inline-block" }}>
      ...
      {open && panelPos && createPortal(
        <div ref={el => { panelRef.current = el; }} style={{
