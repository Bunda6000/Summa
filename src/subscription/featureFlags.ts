export type SubscriptionStatus = 'free' | 'active' | 'grace_period' | 'expired' | 'canceled';

/** Computed access tier derived from raw DB row fields. */
export type SubscriptionTier = 'free' | 'active' | 'grace_period' | 'expired' | 'canceled';
...
export function isFeatureAllowed(tier: SubscriptionTier, featureKey: FeatureKey): boolean {
  const featureTier = FEATURE_FLAGS[featureKey];
  if (featureTier === 'free') return true;
  return tier === 'active' || tier === 'grace_period';
}
