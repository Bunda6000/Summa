...
describe("useSubscriptionStore — trial state", () => {
  it("sets tier to trial when status is trial and trialEndsAt is in the future", async () => {
    mockQuery({
      data: {
        status: "trial",
    ...
    await useSubscriptionStore.getState().initSubscription("user-1");
    expect(useSubscriptionStore.getState().tier).toBe("trial");
    expect(useSubscriptionStore.getState().trialEndsAt).toBe(FUTURE);
    expect(useSubscriptionStore.getState().trialStartedAt).toBe(PAST);
  });

  it("sets tier to expired when status is trial but trialEndsAt has passed", async () => {
    mockQuery({
      data: {
        status: "trial",
        current_period_end: null,
  ...
  it("resets trialEndsAt and trialStartedAt on resetSubscription", async () => {
    mockQuery({
      data: {
        status: "trial",
        current_period_end: null,
    ...
    await useSubscriptionStore.getState().initSubscription("user-1");
    useSubscriptionStore.getState().resetSubscription();
    expect(useSubscriptionStore.getState().trialEndsAt).toBeNull();
    expect(useSubscriptionStore.getState().trialStartedAt).toBeNull();
  });
});
