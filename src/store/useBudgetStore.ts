...
interface BudgetState {
  ...
  // ── Expense actions ──
  setExp: (catId: string, key: string, entry: ExpenseEntry, applyMonths: number) => void;
  delExp: (catId: string, key: string) => void;
  setExpPaid: (catId: string, key: string, paid: boolean, paidDate: string) => void;
  setAllSubsPaid: (catId: string, key: string, paid: boolean, paidDate: string) => void;
  bulkDelExp: (catId: string, keys: string[]) => void;

  // ── Category actions ──
  addCategory: (catData: Omit<Category, 'id'>) => void;
  updateCategory: (idx: number, catData: Partial<Category>) => void;
  deleteCategory: (idx: number) => void;
  reorderCategories: (from: number, to: number) => void;
  updateCatColOrder: (catId: string, colOrder: string[]) => void;
  cleanCategoryUpdate: (idx: number, catData: Partial<Category>, removedSubIds: string[]) => void;

  // ── Loan actions ──
  addLoanType: (lt: Omit<LoanType, 'id'>) => void;
  updateLoanType: (ltId: string, data: Partial<LoanType>) => void;
  deleteLoanType: (ltId: string) => void;
  ...
  updateFixedIncome: (idx: number, fi: Partial<FixedIncome>) => void;
  deleteFixedIncome: (idx: number) => void;

  // ── Variable income actions ──
  addVarIncome: (vi: Omit<VariableIncome, 'id'>) => void;
  updateVarIncome: (idx: number, vi: Partial<VariableIncome>) => void;
  deleteVarIncome: (idx: number) => void;
  bulkDelVarInc: (ids: Set<string>) => void;

  // ── Selectors ──
  getLoanAmountForMonth: (lt: LoanType, monthKey: string) => number;
  getLoansTotalForMonth: (monthKey: string) => number;
  getExp: (catId: string, key: string) => { amount: number } | ExpenseEntry | null;
  ...
  getSuggestedDay: (catId: string) => number;
}
...
const storeCreator: StateCreator<BudgetState, [['zustand/subscribeWithSelector', never]], []> = (set, get) => ({
  ...
  setAllSubsPaid: (catId, key, paid, paidDate) => {
    ...
    const newSubPaid: Record<string, { paid: boolean; paidDate: string }> = {};
    if (paid) {
  ...
  cleanCategoryUpdate: (idx, catData, removedSubIds) => {
    ...
    if (nextExp[catId]) {
      const cleanedEntries: Record<string, ExpenseEntry> = {};
  ...
  getFixedIncomeForMonth: (monthKey) => {
    ...
    fixedIncomes.forEach(src => {
      let applicable: { effectiveFrom: string; amount: number } | null = null;
      (src.records || []).forEach(r => {
        if (r.effectiveFrom <= monthKey && (!applicable || r.effectiveFrom > applicable.effectiveFrom)) applicable = r;
      });
      if (applicable) total += (applicable as { effectiveFrom: string; amount: number }).amount;
    });
    return total;
  },
